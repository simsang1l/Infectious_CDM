    ...
    def transform_cdm(self, source_data, location):
        ...
        try:
            cdm = pd.DataFrame({
                ...
                "place_of_service_source_value": self.place_of_service_source_value 
            })
...
class PersonTransformer(DataTransformer):
    def __init__(self, config_path):
        ...
        self.source_data = self.cdm_config["data"]["source_data"]
        self.source_data_visit = self.cdm_config["data"]["source_data_visit"]
        self.output_filename = self.cdm_config["data"]["output_filename"]
        self.location_source_value = self.cdm_config["columns"]["location_source_value"]
        self.gender_source_value = self.cdm_config["columns"]["gender_source_value"]
    ...
    def process_source(self):
        ...
        try:
            source_data = self.read_csv(self.source_data, path_type = self.source_flag, dtype = self.source_dtype)
            source_data_visit = self.read_csv(self.source_data_visit, path_type = self.source_flag, dtype = self.source_dtype)
            ...
            source_data_visit = source_data_visit[[self.person_source_value, self.race_source_value]]
            source_data_visit.drop_duplicates(inplace = True)
            logging.debug(f"source_data_visit 데이터 row수: {len(source_data_visit)}")

            source_data = pd.merge(source_data, source_data_visit, on= self.person_source_value, how = "left")
    ...
    def transform_cdm(self, source):
        ...
        try : 
            ...
            cdm = pd.DataFrame({
                ...
                "birth_datetime": source[self.birth_date],
                "death_datetime": self.death_datetime,
                "race_concept_id": 0,
...
class VisitOccurrenceTransformer(DataTransformer):
    def __init__(self, config_path):
        ...
        self.admission_end_datetime = self.cdm_config["columns"]["admission_end_datetime"]
        self.emergency_end_datetime = self.cdm_config["columns"]["emergency_end_datetime"]
    ...
    def process_source(self):
        ...
        try : 
            ...
            source[self.admission_end_datetime] = pd.to_datetime(source[self.admission_end_datetime], errors = "coerce")
            source[self.emergency_end_datetime] = pd.to_datetime(source[self.emergency_end_datetime], errors = "coerce")
    ...
    def transform_cdm(self, source):
        ...
        try :
            visit_condition = [
                source[self.visit_source_value].isin(["외래"]),
                source[self.visit_source_value].isin(["입원"]),
                source[self.visit_source_value].isin(["응급"])
            ]
            visit_value = [
                9202,
                9201,
            ...
            end_date_value = [
                source[self.visit_start_datetime].dt.date,
                source[self.admission_end_datetime].dt.date,
                source[self.emergency_end_datetime].dt.date
            ]

            end_datetime_condition = [
                source[self.visit_source_value].isin(["외래"]),
                source[self.visit_source_value].isin(["입원"]),
                source[self.visit_source_value].isin(["응급"])
            ]
            ...
            end_datetime_value = [
                source[self.visit_start_datetime],
                source[self.admission_end_datetime],
                source[self.emergency_end_datetime]
            ]

            cdm = pd.DataFrame({
                "person_id": source["person_id"],
                "visit_concept_id": np.select(visit_condition, visit_value, default = 0),
                "visit_start_date": source[self.visit_start_datetime].dt.date ,
                "visit_start_datetime": source[self.visit_start_datetime],
                "visit_end_date": np.select(end_date_condition, end_date_value, default = source[self.visit_start_datetime].dt.date),
                "visit_end_datetime": np.select(end_datetime_condition, end_datetime_value, default = source[self.visit_start_datetime]),
                "visit_type_concept_id": 44818518,
                "provider_id": source["provider_id"],
                "care_site_id": source["care_site_id"],
                "visit_source_value": source[self.visit_source_value],
                "visit_source_concept_id": np.select(visit_condition, visit_value, default = 0),
...
class ConditionOccurrenceTransformer(DataTransformer):
    def __init__(self, config_path):
        ...
        self.meddept = self.cdm_config["columns"]["meddept"]
        self.pact_tp_nm = self.cdm_config["columns"]["pact_tp_nm"]
    ...
    def process_source(self):
        ...
        try: 
            ...
            # 원천에서 조건걸기
            source = source[[self.person_source_value, self.condition_start_datetime, self.condition_source_value, self.visit_source_key, self.provider, self.pact_tp_nm, self.meddept]]
            ...
            visit_data = visit_data[["visit_occurrence_id", "visit_start_datetime", "care_site_id", "visit_source_value", "person_id", "visit_end_datetime", "visit_source_key"]]
    ...
    def transform_cdm(self, source):
        ...
        try :
            end_date_condition = [
                source[self.pact_tp_nm].isin(["외래"]),
            ]
            end_date_value = [
            ...
            cdm = pd.DataFrame({
                ...
                "condition_end_date": np.select(end_date_condition, end_date_value, default=source["visit_end_datetime"].dt.date),
                "condition_end_datetime": np.select(end_datetime_condition, end_datetime_value, default=source["visit_end_datetime"]),
...
class LocalEDITransformer(DataTransformer):
    ...
    def process_source(self):
        ...
        """
        try : 
            order_data = self.read_csv(self.order_data, path_type = self.source_flag, dtype = self.source_dtype) # 배포시 skiprows=1추가!
    ...
    def process_source(self):
        ...
        try : 
            ...
            source = source[[self.person_source_value, self.drug_source_value, self.drug_exposure_start_datetime, 
                             self.provider, self.days_supply, self.qty, self.cnt, self.dose_unit_source_value,
                             self.hospital, self.route_source_value, self.visit_source_key]]
...
class MeasurementDiagTransformer(DataTransformer):
    def __init__(self, config_path):
        ...
        self.value_as_number = self.cdm_config["columns"]["value_as_number"]
        self.value_as_concept = self.cdm_config["columns"]["value_as_concept"]
    ...
    def process_source(self):
        ...
        try:
            ...
            # 원천에서 조건걸기
            source = source[[self.person_source_value, self.orddate, self.measurement_datetime, self.value_as_number,
                             self.value_as_concept,self.provider, self.hospital, self.value_source_value,
                             self.measurement_source_value, self.visit_source_key]]
            ...
            source[self.value_as_number] = source[self.value_as_number].astype(float)
    ...
    def transform_cdm(self, source):
        ...
        try :
            ...
            value_concept_condition = [
                source[self.value_as_concept] == "+"
                , source[self.value_as_concept] == "++"
                , source[self.value_as_concept] == "+++"
                , source[self.value_as_concept] == "++++"
                , source[self.value_as_concept].str.lower() == "negative"
                , source[self.value_as_concept].str.lower() == "positive"
            ]
            value_concept_value = [
                4123508
                , 4126673
