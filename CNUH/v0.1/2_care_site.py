...
def transform_to_care_site(**kwargs):
    ...
    if "source_data" in kwargs :
        source = pd.read_csv(os.path.join(source_path, kwargs["source_data"]), dtype=str, encoding = 'cp949', skiprows = 1)
    else :
    ...
    cdm = pd.DataFrame({
        ...
        "location_id": location.loc[location["ZIP"] == "614", "LOCATION_ID"].tolist()[0],
...
transform_to_care_site( source_path = "F:\\01.감염병데이터베이스\\data\\cnuh\\emr",
                        CDM_path = "F:\\01.감염병데이터베이스\\data\\cnuh\\cdm",
                        source_data = "PDEDBMSM_부서기본.csv",
