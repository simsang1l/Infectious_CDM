...
def transform_to_measurement_pth(**kwargs):
    ...
    source1 = source1[["병원구분코드", orddate, person_source_value, provider, measurement_date, measurement_source_value, value_source_value, unit_source_value]]
    ...
    # source = pd.merge(source1, source2, left_on=["INSTCD", orddate, "PRCPNO", "PRCPHISTNO"], right_on=["INSTCD", orddate, "PRCPNO", "PRCPHISTNO"], how="inner", suffixes=("", "_diag2"))
    ...
    # source = pd.merge(source, visit_data, left_on=["person_id", "care_site_id", "병원구분코드"], right_on=["person_id", "care_site_id", "visit_start_datetime"], how="left", suffixes=('', '_y'))
    ...
    cdm = pd.DataFrame({
        ...
        "measurement_concept_id": 0, #source["concept_id"],
        ...
        "value_as_number": source[value_source_value],
        ...
        "visit_occurrence_id": 0, #source["visit_occurrence_id"],
        "visit_detail_id": 0,
        "measurement_source_value": source[measurement_source_value],
        "measurement_source_concept_id": None, #source[edicode],
...
transform_to_measurement_pth( source_path = "F:\\01.감염병데이터베이스\\data\\cnuh\\emr",
                               CDM_path = "F:\\01.감염병데이터베이스\\data\\cnuh\\cdm",
                               source_data1 = "MSEPMPMD_병리검사판독결과정보.csv",
