...
def transform_to_provider(**kwargs):
    ...
    source = pd.merge(source, care_site, left_on = [care_site_source_value, "병원구분코드"], right_on=["care_site_source_value", "place_of_service_source_value"], how = "left")
    source[year_of_birth] = pd.to_datetime(source[year_of_birth])
    ...
    cdm = pd.DataFrame({
        ...
        "year_of_birth": source[year_of_birth].dt.year.astype(int),
        "gender_concept_id": 0,
...
transform_to_provider( source_path = "F:\\01.감염병데이터베이스\\data\\cnuh\\emr",
                       CDM_path = "F:\\01.감염병데이터베이스\\data\\cnuh\\cdm",
                       source_data = "PDESMSAM_DAMO_사원정보마스터.csv",
