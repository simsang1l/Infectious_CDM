...
class DataTransformer:
    ...
    def read_csv(self, file_name, path_type = 'source', encoding = 'utf-8', dtype = None, chunksize=None):
        """
        ...
        if chunksize:
            return pd.read_csv(full_path, dtype=dtype, encoding=encoding, chunksize=chunksize)
        else:
            return pd.read_csv(full_path, dtype=dtype, encoding=encoding)

    def write_csv(self, df, file_path, header=True, mode='w'):
        """
        DataFrame을 CSV 파일로 저장합니다.
        """
        df.to_csv(file_path + ".csv", index = False, header = header, mode=mode)
...
class VisitDetailTransformer(DataTransformer):
    ...
    def __init__(self, config_path):
        ...
        self.columns = ["visit_detail_id", "person_id", "환자명", "visit_detail_concept_id", "visit_detail_start_date"
                        ...
                        , "visit_detail_parent_id", "visit_occurrence_id", "진료과", "진료과명"]
    ...
    def process_source(self):
        ...
        try : 
            ...
            source = source[[self.person_source_value, self.drug_source_value, self.drug_exposure_start_datetime,
                             ...
                             self.methodcd, self.age, self.ordseqno]]
            source[self.medtime] = source[self.medtime].apply(lambda x : x[:4] + "-" + x[4:6] + "-" + x[6:8] + " " + x[8:10] + ":" + x[10:])
            source[self.medtime] = pd.to_datetime(source[self.medtime], errors = "coerce")
...
class DrugexposureChunkTransformer(DataTransformer):
    def __init__(self, config_path):
        super().__init__(config_path)
        self.table = "drug_exposure"
        self.cdm_config = self.config[self.table]
        ...
        self.source_data = self.cdm_config["data"]["source_data"]
        self.output_filename = self.cdm_config["data"]["output_filename"]
        self.save_path = os.path.join(self.cdm_path, self.output_filename)
        self.meddept = self.cdm_config["columns"]["meddept"]
        self.provider = self.cdm_config["columns"]["provider"]
        self.drug_exposure_start_datetime = self.cdm_config["columns"]["drug_exposure_start_datetime"]
        self.drug_source_value = self.cdm_config["columns"]["drug_source_value"]
    ...
    def transform(self):
        ...
        try:
            ...
            for chunk in source :
                logging.info(f"원천 데이터 row수:, {len(chunk)}, {self.memory_usage}")
                # 원천 데이터 전처리
                chunk_source_data = self.process_source(chunk, local_edi, person_data, provider_data,
                                                         care_site_data, visit_data, visit_detail, concept_etc)
                # 원천 데이터 CDM으로 변환
    ...
    def process_source(self, source, local_edi, person_data, provider_data,
                       care_site_data, visit_data, visit_detail, concept_etc):
        """
        소스 데이터를 로드하고 전처리 작업을 수행하는 메소드입니다.
        """
...
class MeasurementStexmrstChunkTransformer(DataTransformer):
    def __init__(self, config_path):
        super().__init__(config_path)
        self.table = "measurement_stexmrst"
        self.cdm_config = self.config[self.table]
    ...
    def transform(self):
        ...
        try :
            ...
            for chunk in source:
                logging.debug(f'원천 데이터 row수: {len(chunk)}, {self.memory_usage}')
                chunk_source_data = self.process_source(chunk, local_edi, person_data, provider_data,
                                                        care_site_data, visit_data, visit_detail, concept_etc,
                                                        unit_data, unit_concept_synonym)
                transformed_data = self.transform_cdm(chunk_source_data)
...
class ProcedureOrderChunkTransformer(DataTransformer):
    def __init__(self, config_path):
        super().__init__(config_path)
    ...
    def transform(self):
        ...
        try:
            ...
            for chunk in source :
                logging.debug(f"원천 데이터 row수: {len(chunk)}, {self.memory_usage}")
                chunk_source_data = self.process_source(chunk, local_edi, person_data, provider_data, care_site_data,
                                                        visit_data, visit_detail, concept_etc)
                transformed_data = self.transform_cdm(chunk_source_data)
    ...
    def process_source(self, source, local_edi, person_data, provider_data, care_site_data,
                                                        visit_data, visit_detail, concept_etc):
        """
        소스 데이터를 로드하고 전처리 작업을 수행하는 메소드입니다.
        """
        try: 
            ...
            source = source[[self.person_source_value, self.orddate, self.exectime, self.ordseqno, self.opdate, self.procedure_source_value,
                              self.meddept, self.provider, self.medtime, self.patfg, self.age, self.procedure_source_value_name]]
            logging.debug(f"조건적용 후 원천 데이터 row수: {len(source)}, {self.memory_usage}")
    ...
    def transform_cdm(self, source):
        ...
        try : 
            ...
            cdm = pd.DataFrame({
                ...
                "환자구분": source[self.patfg],
                "잔료과": source[self.meddept],
                "진료과명": source["care_site_name"],
                "나이": source[self.age],
                "처방일": source[self.orddate],
                "수술일": source[self.opdate],
                "처방순번": source[self.ordseqno],
                "결과내역": None, 
                "결론 및 진단": None,
                "결과단위": None
...
class ProcedureStexmrstChunkTransformer(DataTransformer):
    def __init__(self, config_path):
        super().__init__(config_path)
    ...
    def process_source(self, source, local_edi, person_data, provider_data,
        ...
        try:
            ...
            source[self.exectime] = pd.to_datetime(source[self.exectime], format="%Y%m%d%H%M%S", errors = "coerce")
            source[self.medtime] = source[self.medtime].astype(str).apply(lambda x : x[:4] + "-" + x[4:6] + "-" + x[6:8] + " " + x[8:10] + ":" + x[10:])
            source[self.medtime] = pd.to_datetime(source[self.medtime], format="%Y%m%d%H%M%S", errors = "coerce")
