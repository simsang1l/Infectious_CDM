...
from DataTransformer import VisitDetailTransformer

def visit_detail_row_count(config, cdm_path, source_path, excel_path, config_path, sheetname):
    table_name = "visit_detail"
    source_table = config[table_name]["data"]["source_data"]
    patno = config["person_source_value"]
    visit_detail_start_datetime = config[table_name]["columns"]["visit_detail_start_datetime"]
    cdm_date = "visit_detail_start_datetime"

    source = VisitDetailTransformer(config_path).process_source()
    cdm = pd.read_csv(os.path.join(cdm_path, table_name + ".csv"))

    source['year_month'] = source[visit_detail_start_datetime].dt.to_period('M')
