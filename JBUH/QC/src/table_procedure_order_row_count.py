...
from DataTransformer import ProcedureOrderTransformer

def procedure_order_row_count(config, cdm_path, source_path, excel_path, config_path, sheetname):
    table_name = "procedure_order"
    source_table = config[table_name]["data"]["source_data"]
    patno = config["person_source_value"]
    orddate = config[table_name]["columns"]["orddate"]
    cdm_date = "처방일"

    source = ProcedureOrderTransformer(config_path).process_source()
