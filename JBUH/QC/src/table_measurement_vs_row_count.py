...
from DataTransformer import MeasurementVSTransformer

def measurement_vs_row_count(config, cdm_path, source_path, excel_path, config_path, sheetname):
    table_name = "measurement_vs"
    source_table = config[table_name]["data"]["source_data"]
    patno = config["person_source_value"]
    orddate = config[table_name]["columns"]["measurement_datetime"]
    cdm_date = "measurement_datetime"

    source = MeasurementVSTransformer(config_path).process_source()
