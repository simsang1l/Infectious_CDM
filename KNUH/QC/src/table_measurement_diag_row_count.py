"""
사용한 원본데이터와 변환된 CDM measurement_stexmrst테이블의 건수 및 환자수 확인
"""
...
from DataTransformer import MeasurementDiagTransformer

def measurement_stexmrst_row_count(config, cdm_path, source_path, excel_path, config_path, sheetname):
    table_name = "measurement_stexmrst"
    source_table = config[table_name]["data"]["source_data"]
    patno = config["person_source_value"]
    orddate = config[table_name]["columns"]["orddate"]
    cdm_date = "처방일"

    source = MeasurementDiagTransformer(config_path).process_source()
