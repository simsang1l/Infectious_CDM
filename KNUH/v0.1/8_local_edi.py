...
def transform_to_local_edi(**kwargs):
    ...
    if "order_data" in kwargs :
        order_data = pd.read_csv(os.path.join(source_path, kwargs["order_data"]), dtype=str, encoding="cp949")
    else :
        raise ValueError("order_data가 없습니다.")
    
    if "edi_data" in kwargs :
        edi_data = pd.read_csv(os.path.join(source_path, kwargs["edi_data"]), dtype=str, encoding="cp949")
    else :
    ...
    # 처방코드 마스터와 수가코드 매핑
    source = pd.merge(edi_data, order_data, left_on=[sugacode, "INSTCD"], right_on=[ordercode, "INSTCD"], how="left", suffixes=('', '_y'))
    print(len(source))
    ...
    local_edi = source[[sugacode, fromdate, todate, edicode, "INSTCD", "SPCCD", "ENGNM", "TCLSNM", "concept_id", "concept_name", "domain_id", "vocabulary_id", "concept_class_id", "standard_concept", "concept_code", "valid_start_date", "valid_end_date", "invalid_reason"]]
...
transform_to_local_edi( source_path = "F:\\01.감염병데이터베이스\\data\\knuh\\emr",
                        CDM_path = "F:\\01.감염병데이터베이스\\data\\knuh\\cdm",
                        order_data = "23.LLFMTCLS_검체검사.csv", # 검체검사
                        edi_data = "17.PICMMECH_수가마스터.csv", # 수가마스터
