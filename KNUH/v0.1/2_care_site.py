...
def transform_to_care_site(**kwargs):
    ...
    cdm = pd.DataFrame({
        ...
        "location_id": location.loc[location["ZIP"] == "419", "LOCATION_ID"].tolist()[0],
...
transform_to_care_site( source_path = "F:\\01.감염병데이터베이스\\data\\knuh\\emr",
                        CDM_path = "F:\\01.감염병데이터베이스\\data\\knuh\\cdm",
                        source_data = "18.PMCMPTBS_부서코드마스터.csv", # 부서코드 마스터 테이블 입력
                        location_data = "location.csv",
                        care_site_name = "INORDDEPTNM", # 부서명컬럼
                        care_site_source_value = "DEPTCD", # 부서코드 컬럼
                        place_of_service_source_value = "INSTCD" # 기관구분코드 컬럼
)