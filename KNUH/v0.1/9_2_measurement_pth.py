...
def transform_to_measurement_pth(**kwargs):
    ...
    source1 = source1[["INSTCD", "PID", "PTNO", "RSLTRGSTDD", "RSLTRGSTNO", "RSLTRGSTHISTNO", "DELFLAGCD"]]
    source1 = source1[(source1["RSLTRGSTHISTNO"] == "1") & (source1["DELFLAGCD"] == "0") ]
    
    source2 = source2[["INSTCD", "PID", "PTNO", "RSLTRGSTDD", "RSLTRGSTNO", "RSLTRGSTHISTNO", value_source_value]]

    source3 = source3[["INSTCD", "PTNO", "PRCPDD", "PRCPNO", "ACPTSTATCD", measurement_source_value, "SPCCD"]]
    ...
    source4 = source4[["INSTCD", orddate, "PID", "PRCPHISTNO", "ORDDD", "CRETNO", "PRCPCLSCD", "LASTUPDTDT", "ORDDRID", "PRCPNM", "PRCPCD", "PRCPHISTCD", "PRCPNO", "ORDDEPTCD"]]
    source4[orddate] = pd.to_datetime(source4[orddate])
    source4 = source4[(source4[orddate] <= "2023-08-31")]
    source4 = source4[(source4["PRCPHISTCD"] == "O")]

    print('조건 적용후 원천 데이터 개수:', len(source1), len(source2), len(source3), len(source4))
    source = pd.merge(source1, source2, left_on=["INSTCD", "PID", "PTNO", "RSLTRGSTNO", "RSLTRGSTDD", "RSLTRGSTHISTNO"], right_on=["INSTCD", "PID", "PTNO", "RSLTRGSTNO", "RSLTRGSTDD", "RSLTRGSTHISTNO"], how="inner", suffixes=("", "_pth2"))
    print('원천 병합 후 데이터 개수:', len(source))
    del source1
    del source2

    # 각 TABLE JOIN 하기
    source = pd.merge(source, source3, left_on=["INSTCD", "PTNO"], right_on=["INSTCD", "PTNO"], how="inner", suffixes=("", "_pth3"))
    print(source)
    print('원천 병합 후 데이터 개수:', len(source))
    source = pd.merge(source, source4, left_on=["INSTCD", "PRCPDD", "PRCPNO"], right_on=["INSTCD", "PRCPDD", "PRCPNO"], how="inner", suffixes=("", "_pth4"))
    print('원천 병합 후 데이터 개수:', len(source))
    del source3
    del source4
    print(source)
    ...
    source = pd.merge(source, local_edi, left_on=[measurement_source_value, "INSTCD"], right_on= [sugacode, "INSTCD"], how="left")
    ...
    # csv파일로 저장
    cdm.to_csv(os.path.join(CDM_path, "measurement_pth.csv"), index=False)
...
transform_to_measurement_pth( source_path = "F:\\01.감염병데이터베이스\\data\\knuh\\emr",
                               CDM_path = "F:\\01.감염병데이터베이스\\data\\knuh\\cdm",
                               source_data1 = "03.LPRMRSLT_병리검사결과.csv",
                               source_data2 = "27.LPRMCNTSD_병리검사결과.csv",
                               source_data3 = "26.LPJMACPT_병리접수.csv",
                               source_data4 = "02.MMOHIPRC_검사처방.csv",
